...
class MapValuesToCollectionConverterTest {
    ...
    void matches_returns_true_when_converting_from_Map_to_Collection_and_ConversioService_can_convert() {
        final TypeDescriptor sourceType = TypeDescriptor.map(HashMap.class,
                TypeDescriptor.valueOf(Integer.class), TypeDescriptor.valueOf(Long.class));
        final TypeDescriptor sourceValuesType = TypeDescriptor.collection(Collection.class, sourceType.getMapValueTypeDescriptor());
    ...
    void convert_delegates_to_ConversionService() {
        final TypeDescriptor sourceType = TypeDescriptor.map(HashMap.class,
                TypeDescriptor.valueOf(Integer.class), TypeDescriptor.valueOf(Long.class));
        final TypeDescriptor mapValueTypeDescriptor = sourceType.getMapValueTypeDescriptor();
        final TypeDescriptor sourceValuesType = TypeDescriptor.collection(Collection.class, mapValueTypeDescriptor);
    ...
    void convert_delegates_to_ConversionService_for_conversion_of_nested_types() {
        final TypeDescriptor sourceType = TypeDescriptor.map(HashMap.class,
                TypeDescriptor.collection(HashSet.class, TypeDescriptor.valueOf(Integer.class)),
                TypeDescriptor.collection(ArrayList.class, TypeDescriptor.valueOf(Long.class)));
        final TypeDescriptor sourceValuesType = TypeDescriptor.collection(Collection.class, sourceType.getMapValueTypeDescriptor());
