...
import fi.fmi.avi.archiver.config.model.ConfigurableComponentInstanceSpec;
...
@Component
public class ConfigurableComponentsUtil {
    ...
    public ConfigurableComponentsUtil(final ConfigValueConverter configValueConverter, final ConditionPropertyReaderFactory conditionPropertyReaderFactory) {
    ...
    public <T, C extends T, S extends ConfigurableComponentInstanceSpec> Stream<T> createConditionalComponents(
        ...
        return specs.stream()
                .map(spec -> createConditionalComponent(spec, factoriesByName, conditionalComponentFactory));
    }

    public <T, C extends T, S extends ConfigurableComponentInstanceSpec> T createConditionalComponent(
            final S spec,
        ...
        return applyComponentActivationCondition(component, spec, conditionalComponentFactory);
    }

    @SuppressWarnings({"rawtypes", "unchecked"})
    public <T, C extends T, S extends ConfigurableComponentInstanceSpec> T applyComponentActivationCondition(
            final T component,
            final S spec,
            final BiFunction<ActivationCondition, T, C> conditionalComponentFactory) {
